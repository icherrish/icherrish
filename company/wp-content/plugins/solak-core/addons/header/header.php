 ...
 *
 */
class Solak_Header extends Widget_Base {

	public function get_name() {
		return 'solakheader';
	}
	public function get_title() {
		return __( 'Header', 'solak' );
	}
	...
	protected function register_controls() {
		...
		solak_general_fields($repeater, 'content', 'Content', 'TEXTAREA', 'Content here');
		...
		solak_general_fields($this, 'social_label', 'Social Label', 'TEXT', 'Follow Us On:', '1');
		solak_social_fields($this, 'social_lists', 'Social Lists', ['1']);
		...
		solak_general_fields( $this, 'button_text', 'Button Text', 'TEXT', 'Free consultation', ['1'] );
		...
		solak_color_fields( $this, '01', 'Topbar Background', 'background', '{{WRAPPER}} .header-top', ['1'] );
		solak_color_fields( $this, '02', 'Menu Background', 'background', '{{WRAPPER}} .header-layout2 .menu-area:after, {{WRAPPER}} .header-layout2 .menu-area:before', ['1'] );                
		...
		solak_color_fields( $this, 'menu_color1', 'Color', 'color', '{{WRAPPER}} .main-menu>ul>li>a', ['1'] );
		solak_color_fields( $this, 'menu_color2', 'Hover Color', 'color', '{{WRAPPER}} .main-menu>ul>li>a:hover', ['1'] );
	...
	protected function render() {
		...
		if( $settings['layout_style'] == '1' ){
			echo '<div class="th-header header-layout2 header-absolute">';
									...
									echo '<div class="header-links">';
										echo '<ul>';
											foreach ( $settings['contact_info'] as $index => $data ) {
												$class = $index === 0 ? 'd-none d-xl-inline-block' : '';
												if(!empty($data['content'])){
													echo '<li class="' . esc_attr( $class ) . '">' . wp_kses_post( $data['content'] ) . '</li>';
												}
											}
										echo '</ul>';
									echo '</div>';
									...
									echo '<div class="col-auto">';
										echo '<div class="social-links">';
											if(!empty($settings['social_label'])){
												echo '<span class="social-title">' .esc_html( $settings['social_label'] ). '</span>';
											}
							...
							echo '<div class="row align-items-center justify-content-between">';
								...
								echo '<div class="col-auto d-none d-xl-block">';
									echo '<div class="header-button">';
										...
										if(!empty($settings['show_offcanvas_btn'])){
											echo '<a href="#" class="icon-btn sideMenuToggler d-none d-lg-block"><img src="'.SOLAK_ASSETS.'/img/grid.svg"></a>';
										}
										if(!empty( $settings['button_text'])){
											echo '<a href="'.esc_attr($settings['button_url']['url']).'" class="th-btn style2 th-icon"><span class="btn-text" data-back="'.esc_html($settings['button_text']).'" data-front="'.esc_html($settings['button_text']).'"></span><i class="fa-regular fa-arrow-right ms-2"></i> </a>';
										}
									echo '</div>';
								echo '</div>';
							echo '</div>';
