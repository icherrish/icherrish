...
<?php $component->withAttributes([]); ?>
    ...
    <div class="container-xl px-4">
        <div class="card mb-4">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-8"><?php echo e($module->name); ?></div>
                    <div class="col-md-4">
                        <div class="input-group input-group-joined border-0 add-button">
                            <?php if($module->is_preview): ?>
                      <?php endif; ?><a class="btn btn-danger btn-sm" href="<?php echo e(url('/admin/'.$module->slug.'/add')); ?>"><?php echo e(__('Add New '.$module->term)); ?></a>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                ...
                <?php endif; ?>
                <input type="hidden" id="table-modules-data-url" value="<?php echo e(Request::url()); ?>">
                <input type="hidden" id="is_enable_modules_data_action" value="<?php echo e((in_array('modules.data.edit', permissions()) || in_array('modules.data.destroy', permissions())) ? 'yes' : 'no'); ?>">
                <table class="table table-bordered" id="table-modules-data">
                    <thead>
                        <tr>
                            ...
                            <th>
                            <?php echo e($module->term); ?>

                                
                            </th>
                            
                            <?php if($module->category): ?>
                                <th><?php echo $parent->term; ?></th>
                            <?php endif; ?>
                            <?php $__currentLoopData = $module->fields()->get() ?? [__('Created Date')]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <th><?php echo e($list->field_title ?? __('Created Date')); ?></th>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <th><?php echo e(__('Status')); ?></th>
                            

                            <th style="<?php if($module->is_preview): ?> width: 120px; <?php endif; ?>"><?php echo e(__('Action')); ?></th>
                        </tr>
                        <tr role="row" class="filter">
                            ...
                            <?php if($module->category): ?>
                                <td><?php echo Form::select('category', [''=>'Select '.$parent->term]+dataArray($module->parent_id), null, array('class'=>'form-control', 'id'=>'category', 'required'=>'required')); ?></td>
                            <?php endif; ?>
                            <?php $__currentLoopData = $module->fields()->get() ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <td>
                                    <?php if($list->field_type == 'select'): ?>
                                    <?php echo Form::select($list->field, [''=>'Select '.$list->field_title]+dataArray($list->field_attr), null, array('class'=>'form-control', 'id'=>$list->field, 'required'=>'required')); ?>

                                    <?php else: ?>
                                    <input type="text" class="form-control" name="<?php echo e($list->field); ?>" id="<?php echo e($list->field); ?>" autocomplete="off" placeholder="Search <?php echo e($list->field_title); ?>">
                                    <?php endif; ?>
                                </td>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <input type="hidden" id="is_image" name="is_image" value="<?php echo e($module->is_image ? 'yes' : 'no'); ?>">
    <input type="hidden" id="is_preview" name="is_preview" value="<?php echo e($module->is_preview ? 'yes' : 'no'); ?>">
    <input type="hidden" id="is_category" name="is_category" value="<?php echo e($module->category ? 'yes' : 'no'); ?>">
    <input type="hidden" id="message_added" name="message_added" value="<?php echo e(session()->has('message.added') ? 'yes' : 'no'); ?>">
    <input type="hidden" id="message" name="message" value="<?php echo session('message.content'); ?>">
    <input type="hidden" id="modules_data_fetch" name="modules_data_fetch" value="<?php echo e(route('admin.modules.data.fetch')); ?>">
    <input type="hidden" id="module_id" name="module_id" value="<?php echo e($module->id); ?>">
    <input type="hidden" id="module_fields" name="module_fields" value="<?php echo e(json_encode($module->fields()->get())); ?>">
    <input type="hidden" id="base_url" name="base_url" value="<?php echo e(url('/')); ?>">
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="<?php echo e(route('admin.assign-contacts')); ?>" method="post">
                        <?php echo csrf_field(); ?>
                            ...
                            <tbody>
                                <?php $users = App\Models\User::where('role',4)->get(); ?>
                                 ...
                                 <tr>
                                     ...
                                     <td><input type="radio" value="<?php echo e($user->id); ?>" name="selected_users" class="selected_users"></td>
                                 </tr>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="import-contacts" action="<?php echo e(route('admin.import.contacts')); ?>" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
    ...
    <?php $__env->startPush('js'); ?>
        <script src="<?php echo e(asset('admin_assets/js/modules.js?v=4.2')); ?>"></script>
        <script>
        ...
        $('.btn-import-contacts').click(function () {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo e(route('admin.import.contacts')); ?>',
                data: formData,
                contentType: false,
                processData: false,
...
<?php endif; ?>
<?php /**PATH /home/icherrish.com/public_html/easytrips/resources/views/admin/modules_data/index.blade.php ENDPATH**/ ?>